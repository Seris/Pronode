...
module.exports = function(app){
    ...
    logger.log('Tentative de lancement de l\'application :', app.name);
    ...
    try{
        ...
        logger.warn('Erreur dans la configuration de l\'application ' + app.name.underline);
    ...
    if(!fs.existsSync(config.entry_point)){
        logger.warn("Le entry_point de l'application", app.name.underline, "n'existe pas !");
    ...
    CurrentApp.on('start', function(err, app){
        CurrentApp.process.on('exit', function(code, signal){
            logger.info("L'application", CurrentApp.name.underline, "s'est stoppée avec le code", code, '[' + signal + ']');
            delete Applications[CurrentApp.name];
        });

        if(err){
            logger.warn("Erreur lors du lancement de l'application", app.name.underline + '\n\r' + err.stack);
        ...
        logger.info("L'application", CurrentApp.name.underline, 'a bien été lancé ! [pid=' + CurrentApp.process.pid + ']');
